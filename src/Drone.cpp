...
*/
    ...
    */
    Drone::Drone(int nr_drona,PzG::LaczeDoGNUPlota &Lacze, Vector3D wspolrzende):Lacze(Lacze)
    {
        ...
        Lacze.DodajNazwePliku(body_org.jaka_nazwa().c_str(), PzG::RR_Ciagly, 2);

        for(int i=0; i<4; i++)
        {
        Lacze.DodajNazwePliku(wings_org[i].jaka_nazwa().c_str(), PzG::RR_Ciagly, 2);
        }
...
*/
    void Drone::pilot()
    {
        ...
        case 'l':
            ...
            std::cin >> droga;
            narysuj_trase(droga);
            Lacze.DodajNazwePliku("../datas/trajektoria.dat", PzG::RR_Ciagly, 2);
            for (int i=0;i<100; i++)
            {
            body_copy=body_org;
            for (int j = 0; j < 4; j++)
